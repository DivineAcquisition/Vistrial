...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <OrgProvider value={toClientOrgState(ctx)} key={ctx.org.id}>
      <div className="app-type relative min-h-screen bg-ink-950 text-card-foreground">
        <header className="sticky top-0 z-20 border-b border-border bg-ink-950/90 backdrop-blur">
          <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 px-5 py-3 sm:px-8">
            <Link href="/portal" className="flex min-w-0 items-center gap-3" aria-label={`${APP_NAME} owner portal`}>
