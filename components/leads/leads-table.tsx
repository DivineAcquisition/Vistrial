  ...
  return (
    <>
          ...
          <TableBody>
            {rows.map((lead) => (
              <TableRow
                ...
                className="cursor-pointer border-white/[0.05] hover:bg-white/[0.02] focus-visible:bg-white/[0.03] focus-visible:outline-none"
              >
                <TableCell className="px-4 py-3.5 whitespace-nowrap text-neutral-300 tabular-nums">
                  {formatDateTime(lead.arrivedAt)}
                </TableCell>
                <TableCell className="px-4 py-3.5 text-neutral-300">
                  {lead.clientName}
                </TableCell>
                ...
                <TableCell className="px-4 py-3.5 text-neutral-400">
                  <span className="block tabular-nums">{lead.phone ?? DASH}</span>
                  {lead.email ? (
                    <span className="block text-xs text-neutral-500">{lead.email}</span>
                  ) : null}
                </TableCell>
                ...
                <TableCell className="px-4 py-3.5">
                  {lead.appointment ? (
                    ...
                    <span className="text-neutral-600">{DASH}</span>
                  )}
                </TableCell>
              </TableRow>
            ))}
      ...
      >
        <SheetContent
          side="right"
          className="w-full overflow-y-auto border-white/[0.08] bg-ink-900 sm:max-w-xl"
        >
      ...
      <div className="space-y-7 px-5 pb-8">
          ...
          {lead.touches.length === 0 ? (
            ...
            <ol className="space-y-2">
              {lead.touches.map((touch) => (
                <li
                  key={touch.id}
                  className="flex flex-wrap items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 py-2.5"
                >
        ...
        <section>
          ...
          <ol className="space-y-2">
            {lead.submissions.map((submission) => (
              <li
                key={submission.id}
                className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 py-2.5"
              >
        ...
        <section>
          ...
          <details className="rounded-xl border border-white/[0.06] bg-white/[0.02]">
            <summary className="cursor-pointer px-3.5 py-2.5 text-sm text-neutral-400 select-none">
              Show the payload exactly as it arrived
            </summary>
            <pre className="max-h-80 overflow-auto border-t border-white/[0.06] px-3.5 py-3 text-xs leading-relaxed text-neutral-400">
              {lead.payload}
            </pre>
          </details>
        </section>
...
function SheetSectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="mb-3 text-[11px] font-semibold tracking-[0.18em] text-neutral-500 uppercase">
      {children}
    </h3>
  );
}
