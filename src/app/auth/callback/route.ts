...
export async function GET(request: NextRequest) {
  ...
  // Create a Supabase client that can write cookies in the response
  ...
  // --- Flow 1: OAuth code exchange (Google, etc.) ---
  if (code) {
  ...
  // --- Flow 2: Email verification / magic link (token_hash) ---
  if (token_hash && type) {
    const { data, error } = await supabase.auth.verifyOtp({
      token_hash,
      type: type as 'email' | 'signup' | 'recovery' | 'magiclink' | 'email_change',
    });
...
// ============================================
// Helper: ensure user has an organization
// ============================================
async function ensureOrganization(user: { id: string; email?: string; user_metadata?: Record<string, unknown> }) {
  try {
    ...
    if (membership) return; // already has one
